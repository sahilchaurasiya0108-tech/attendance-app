          ...
          {employees.map(emp => (
            <div key={emp._id} className="card p-4 flex items-center gap-4 animate-slide-up">
              ...
              <div className="flex gap-2 flex-shrink-0">
                <button onClick={() => setModal(emp)} className="p-2 rounded-lg text-gray-400 hover:text-brand-500 hover:bg-brand-500/10 transition-all">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                </button>
                <button onClick={() => toggleStatus(emp)} className={`p-2 rounded-lg transition-all ${emp.isActive ? 'text-amber-400 hover:bg-amber-500/10' : 'text-emerald-400 hover:bg-emerald-500/10'}`}>
                  {emp.isActive
                    ? <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" /></svg>
                    : <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                  }
                </button>
                <button onClick={() => deleteEmployee(emp)} className="p-2 rounded-lg text-red-400 hover:bg-red-500/10 transition-all" title="Delete employee">
